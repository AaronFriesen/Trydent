...
public class InterpolationExample extends JComponent {
    ...
    public static void main(String[] args) {
        ...
        Position[] pointsA = {
            new Position(60, 60), new Position(150, 60), new Position(250, 150), new Position(350, 100),
            new Position(450, 350), new Position(550, 200)
        };

        example.render(Color.BLACK,
                new SplineCurve<BaseVector<?>>(Interpolation.STRAIGHT, IndexWrapMode.CLAMP, pointsA), pointsA);
        example.render(Color.RED, new SplineCurve<BaseVector<?>>(Interpolation.SMOOTH, IndexWrapMode.REFLECT, pointsA),
                pointsA);
        example.render(Color.GREEN, new SplineCurve<BaseVector<?>>(Interpolation.SMOOTH, IndexWrapMode.WRAP, pointsA),
                pointsA);
        example.render(Color.BLUE, new SplineCurve<BaseVector<?>>(Interpolation.SMOOTH, IndexWrapMode.CLAMP, pointsA),
                pointsA);

        Position[] pointsB = {
            new Position(100, 400), new Position(100, 500), new Position(200, 600), new Position(300, 500),
            new Position(200, 400),
        };
        example.render(Color.BLACK, true, new SplineCurve<BaseVector<?>>(Interpolation.STRAIGHT, IndexWrapMode.WRAP,
                pointsB), pointsB);
        example.render(Color.RED, true, new SplineCurve<BaseVector<?>>(Interpolation.SMOOTH, IndexWrapMode.REFLECT,
                pointsB), pointsB);
        example.render(Color.GREEN, true, new SplineCurve<BaseVector<?>>(Interpolation.SMOOTH, IndexWrapMode.WRAP,
                pointsB), pointsB);
        example.render(Color.BLUE, true, new SplineCurve<BaseVector<?>>(Interpolation.SMOOTH, IndexWrapMode.CLAMP,
                pointsB), pointsB);
    ...
    private void render(final Color color, final boolean nPlus1, final Curve<BaseVector<?>> curve,
            final BaseVector<?>... points) {
        addRenderAction(new Runnable() {
            @Override
            public void run() {
