...
public class AnimationExample implements Runnable {
    ...
    private void createAnimation(Position pos, Color color, IndexWrapMode indexWrap, TimeWrapMode timeWrap,
        ...
        Animation anim = KeyframeAnimation.create().setOrientation(obj1.getOrientation()).addKeyframe(1)
                .setOrientation(obj2.getOrientation()).addKeyframe(1).setOrientation(obj3.getOrientation())
                .addKeyframe() // final keyframe has no duration
                .setAnimationCircular(circular) // false by default
                .build();
    ...
    private void createSpinny(Position pos) {
        ...
        Animation anim = KeyframeAnimation.create().setPosition(pos).addKeyframe().rotateBy(90)
                .setScale(new Scale(1.5)).addKeyframe().setScale(new Scale(1)).rotateBy(90).addKeyframe().rotateBy(90)
                .setScale(new Scale(1.5)).addKeyframe().setScale(new Scale(1)).setTotalDuration(2)
                .setAnimationCircular(true).build();
    ...
    private void createWheely(Position pos, int size) {
        ...
        Animation anim = KeyframeAnimation.create().setPosition(pos).addKeyframe().rotateBy(90)
                .moveBy(new Position(0, -50)).addKeyframe().rotateBy(90).moveBy(new Position(-50, 0)).addKeyframe()
                .rotateBy(90).moveBy(new Position(0, 50)).addKeyframe().setTotalDuration(2).setAnimationCircular(true)
                .build();
